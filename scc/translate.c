...
//   * OP_BR whose cond is not an OP_SET_XXX
//   * load/store -> specific for size 8, 16, 32 (and 64) and other?
//   * idem for cast with sign extension -> OP_SCAST8, ...
//   * split unconditional OP_BR into OP_BR and OP_CBR ?
...
static struct cg_state *get_src_state(struct basic_block *bb, pseudo_t src)
{
	...
	switch (src->type) {
	case PSEUDO_REG:
		if (insn && insn->bb == bb && ptr_list_size((struct ptr_list *)src->users) == 1) {
			s = insn->cg_state;
...
static int is_pow2(pseudo_t v)
{
	unsigned long long val = v->value;

	if (v->type != PSEUDO_VAL)
		return 0;
...
static void translate_binop(struct instruction *insn)
{
	...
	s->kids[0] = get_src_state(insn->bb, src1);
	s->kids[1] = get_src_state(insn->bb, src2);
...
static void translate_cast(struct instruction *insn)
{
	...
	if (new_size < old_size) {
		...
		int sign = insn->type->ctype.modifiers & MOD_SIGNED;
		int rsized = (new_size == bits_in_int || new_size == bits_in_pointer);

#if 1
		//printf("cast: size: old = %u, new = %u\n", old_size, new_size);
		//printf("cast: op = %d, sign = %d\n", op, !!sign);
		sign = (op == INSN_SCAST);
		if (new_size <= (bits_in_pointer * 2))
			rsized = 1;
#endif
		if (rsized && old_size == 8)
			op = sign ? INSN_SEXTB : INSN_ZEXTB;
		else if (rsized && old_size == 16)
			op = sign ? INSN_SEXTH : INSN_ZEXTH;
		else if (rsized && old_size == 32)
			op = sign ? INSN_SEXTL : INSN_ZEXTL;
		else if (rsized && old_size == 64)
			op = sign ? INSN_SEXTQ : INSN_ZEXTQ;
		else {
			op = sign ? INSN_SCAST : INSN_CAST;
			val = old_size;
		}
	}
...
static void translate_computed_goto(struct instruction *insn)
{
	...
	s->kids[0] = get_src_state(insn->bb, insn->target);
	label_state(s);
}
