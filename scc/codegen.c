...
struct ptree *mktree(const char *name, int size, struct ptree *left, struct ptree *right, struct ptree *extra)
{
	struct ptree *t;
	int arity = 0;
...
static struct nterm *mkrule_internal(int lineno, struct ptree *t)
{
	...
	foreach_rule(r) {
		if (r->cost != 0)	// not an internal rule
			continue;
		...
		if (r->arity != t->arity)
			continue;
		for (i = 0; i < r->arity; i++) {
			if (!lookup_nterm(t->kids[i]->name))
				goto next;
			if (strcmp(r->kids[i]->name, t->kids[i]->name))
				goto next;
		}
		// found an identical rule
		return r->lhs;
	      next:;
	}
	// no identical rule found
